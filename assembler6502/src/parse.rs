...
pub enum Line {
    ...
    DataWord {
        label: Option<String>,
        expr: String,
    }, // Changed from value:u16 to expr:String
    Block {
        label: Option<String>,
        expr: String,
    }, // Changed from size:usize to expr:String for symbol support
}
...
pub fn lex_line(raw: &str) -> Line {
    ...
    if let Some(cap) = RE_ORG_PLAIN.captures(s) {
        let val = &cap[1];
        let v = if let Some(hex) = val.strip_prefix('$') {
    ...
    if let Some(mnem) = parts.next() {
        ...
        if matches!(upper.as_str(), "IFN" | "IFE" | "IF1" | "IF2" | "IFNDEF") {
            // Preprocessor should have removed these; if a label was present, preserve it
        ...
        let operand_opt = if operand.is_empty() {
            None
        } else {
            Some(operand)
        };
