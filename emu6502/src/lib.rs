...
impl Cpu {
    ...
    pub fn step(&mut self) {
        ...
        if self.halted {
            return;
        } // interrupt may set halted (if vector 0 and trap)
        let op = self.fetch8();
        ...
        match op {
            ...
            0x10 => if self.branch_with_page(self.p & FLAG_N == 0, &mut extra_cycles) {},
            0x30 => if self.branch_with_page(self.p & FLAG_N != 0, &mut extra_cycles) {},
            0x50 => if self.branch_with_page(self.p & FLAG_V == 0, &mut extra_cycles) {},
            0x70 => if self.branch_with_page(self.p & FLAG_V != 0, &mut extra_cycles) {},
            0x90 => if self.branch_with_page(self.p & FLAG_C == 0, &mut extra_cycles) {},
            0xB0 => if self.branch_with_page(self.p & FLAG_C != 0, &mut extra_cycles) {},
            0xD0 => if self.branch_with_page(self.p & FLAG_Z == 0, &mut extra_cycles) {},
            0xF0 => if self.branch_with_page(self.p & FLAG_Z != 0, &mut extra_cycles) {},
            ...
            _ => {
                panic!("unimplemented opcode {:02X} @{:04X}", op, self.pc - 1);
            }
        }
    ...
    fn base_cycles(op: u8) -> u8 {
        match op {
            ...
            0xA5 | 0xA6 | 0xA4 | 0x85 | 0x86 | 0x84 | 0x05 | 0x25 | 0x45 | 0x24 | 0xC5 | 0xE4
            | 0xC4 | 0x65 | 0xE5 => 3,
            ...
            // Absolute
            0xAD | 0xAE | 0xAC | 0x8D | 0x0D | 0x2D | 0x4D | 0x2C | 0xCD | 0xEC | 0xCC | 0x6D
            | 0xED => 4,
            ...
            0x18 | 0x38 | 0x58 | 0x78 | 0xB8 | 0xD8 | 0xF8 | 0xAA | 0x8A | 0xA8 | 0x98 | 0xBA
            | 0x9A | 0xE8 | 0xC8 | 0xCA | 0x88 | 0xEA | 0x00 | 0x0A | 0x4A | 0x2A | 0x6A => 2,
    ...
    fn write(&mut self, addr: u16, v: u8) {
        ...
        let mut hit = None;
        for (i, (a, _)) in self.write_hooks.iter().enumerate() {
            if *a == addr {
                hit = Some(i);
                break;
            }
    ...
    fn interrupt(&mut self, vector: u16) {
        ...
        if self.pc == 0 {
            // safety: if vector empty and trap_brk emulate halt
            self.halted = true;
        }
    ...
    fn sbc(&mut self, v: u8) {
        ...
        let mut result = sum2;
        let mut c_out = c1 || c2; // this is correct carry (no borrow)
        if self.p & FLAG_D != 0 {
            // BCD SBC correction
...
mod tests {
    ...
    fn load_store() {
        let mut c = base_cpu();
        c.load(0x0800, &[0xA9, 0x10, 0x85, 0x02, 0xA5, 0x02, 0x00]);
        c.reset();
        while !c.halted {
            c.step();
        }
    ...
    #[test]
    fn adc_sbc() {
        let mut c = base_cpu(); // A=5; ADC #3 => 8; SEC; SBC #1 => 7
        c.load(0x0800, &[0xA9, 0x05, 0x69, 0x03, 0x38, 0xE9, 0x01, 0x00]);
        c.reset();
        while !c.halted {
            c.step();
        }
    ...
    #[test]
    fn branches() {
        let mut c = base_cpu(); // BNE skip BRK (offset 1), then execute LDA #5
        c.load(0x0800, &[0xA9, 0x01, 0xD0, 0x01, 0x00, 0xA9, 0x05, 0x00]);
        c.reset();
        while !c.halted {
            c.step();
        }
    ...
    fn compare() {
        let mut c = base_cpu();
        c.load(0x0800, &[0xA9, 0x05, 0xC9, 0x03, 0x00]);
        c.reset();
        while !c.halted {
            c.step();
        }
    ...
    fn decimal_adc() {
        ...
        c.mem[0xFFFE] = 0x00;
        c.mem[0xFFFF] = 0x10;
        c.load(0x0800, &[0xF8, 0xA9, 0x15, 0x69, 0x27, 0x00]); // SED; LDA #$15; ADC #$27 -> $42 (BCD)
        c.reset();
        while !c.halted {
            c.step();
            if c.cycles > 50 {
    ...
    #[test]
    fn irq_basic() {
        let mut c = base_cpu(); // keep BRK as trap for halt
                                // IRQ vector points to handler: LDA #$33; BRK (trap halt)
        c.mem[0xFFFE] = 0x00;
        c.mem[0xFFFF] = 0x09;
        c.load(0x0900, &[0xA9, 0x33, 0x00]);
    ...
    fn write_hook() {
        ...
        c.load(0x0800, &[0xA9, 0x41, 0x8D, 0x01, 0xF0, 0x00]);
        c.reset();
    ...
    fn page_cross_indexed_read_cycles() {
        let mut c = base_cpu(); // Place LDA $10FF,X with X=1 crossing into $1100
        ...
        assert_eq!(c.cycles - start, 9);
        assert_eq!(c.a, 0x55);
    }
    #[test]
    fn branch_page_cross_cycle() {
        let mut c = base_cpu(); // BNE forward crossing page boundary
                                // Put code at $08FE: LDA #1; BNE +2 (to $0903 after page cross); BRK; NOP; BRK
        c.load(0x08FE, &[0xA9, 0x01, 0xD0, 0x02, 0x00, 0xEA, 0x00]);
        c.mem[0xFFFC] = 0xFE;
        c.mem[0xFFFD] = 0x08;
        c.reset();
        while !c.halted {
